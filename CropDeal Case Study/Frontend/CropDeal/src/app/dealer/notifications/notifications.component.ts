...
export class NotificationsComponent implements OnInit {
  ...
  public grainsMessage = "";
  public fruitsMessage = "";
  public vegetablesMessage = "";
  public nutsMessage = "";
  ...
  constructor(private _auth: AuthService, private _router: Router, private _cropService: CropService
    ...
    NotificationsComponent.Crop_Service = this._cropService;
  }
  ...
  async sortBasedOnCropType() {
    ...
    if (this.grainsArray.length == 0) {
      this.grainsMessage = "Crop not yet available or you have not subscribed to this crop";
    }
    if (this.fruitsArray.length == 0) {
      this.fruitsMessage = "Crop not yet available or you have not subscribed to this crop";
    }
    if (this.vegetablesArray.length == 0) {
      this.vegetablesMessage = "Crop not yet available or you have not subscribed to this crop";
    }
    if (this.nutsArray.length == 0) {
      this.nutsMessage = "Crop not yet available or you have not subscribed to this crop";
    }
  ...
  OpenRazorPay() {
    ...
    this.purchaseCropForm.value.pricePerKg = this.cropObj.pricePerKg;
    NotificationsComponent.formValues = this.purchaseCropForm.value;
    //console.log(this.purchaseCropForm.value);
    ...
    var options = {
      ...
      "handler": (response) => {
        ...
        NotificationsComponent.Crop_Service.purchaseCrop(NotificationsComponent.formValues.publishedCropId, NotificationsComponent.formValues)
          .subscribe(
            res => {
              //console.log(res)
              if (res.success) {
  ...
  ngOnInit(): void {
    ...
    this._cropService.getAllSubscribedCrops(this.userNameAndEmailObj.userEmail)
      .subscribe(
        res => {
          //console.log(res)
          if (res.success) {
